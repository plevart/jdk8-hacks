...
public class HashMapArrayAccessor4 extends AbstractMapArrayAccessor {
    ...
    public int getArrayLength(Map<?, ?> map) {
        ...
        while ((length & 3) > 0) // must be a multiple of 4
        {
            length++;
        }
        return length;
    ...
    public Object[] toArray(Map<?, ?> map, Object[] array, int offset, int length) {
        ...
        for (Iterator<? extends Map.Entry<?, ?>> it = map.entrySet().iterator(); it.hasNext(); ) {
            ...
            int h = ((hash(key) << 1) & Integer.MAX_VALUE) % keysLength;
            int ki = offset + h;
            if (array[ki] == null) {
                array[ki] = key;
                array[ki + keysLength] = value;
            }
            else {
    ...
    protected int indexOf(Object[] array, int offset, int keysLength, Object key) {
        ...
        int ki = offset + h;
        // 1st try exact even index (most keys will be put at exact locations)
        Object value = array[ki];
        if (value == null) return -1; // found gap - no match
        if (key.equals(value)) return ki; // match
        // 2nd try odd displaced indexes (that do not clash with exact even locations)
        for (int dh = 1; dh < keysLength; dh+=2) {
            int hi = h + dh;
            if (hi >= keysLength) hi -= keysLength;
            ki = offset + hi;
            value = array[ki];
        ...
        // 3rd try even displaced indexes (they can clash with exact even locations therefore they are tried last)
        for (int dh = 2; dh < keysLength; dh+=2) {
            int hi = h + dh;
            if (hi >= keysLength) hi -= keysLength;
            ki = offset + hi;
            value = array[ki];
